...
public class SpatialGrid {
    ...
    public static class GridNode {
        ...
        GridNode(CollidableEntity entity, int xIndex, int yIndex) {
            this.entity = entity;
            this.xIndex = xIndex;
            this.yIndex = yIndex;
        }
    ...
    public void addEntity(CollidableEntity entity, boolean isStatic) {
    ...
    public void updateEntity(CollidableEntity entity) {
    ...
    public List<CollidableEntity> getNearbyEntities(CollidableEntity entity) {
        int[] cellIndicesMin = getCellIndices(entity, true);
        int[] cellIndicesMax = getCellIndices(entity, false);

        Set<CollidableEntity> nearbyEntities = new HashSet<>();
        queryId++;
        ...
        for (int x = cellIndicesMin[0]; x <= cellIndicesMax[0]; x++) {
            for (int y = cellIndicesMin[1]; y <= cellIndicesMax[1]; y++) {
                if (x >= 0 && x < totalCellsX && y >= 0 && y < totalCellsY) {
                    ...
                    while (node != null) {
                        CollidableEntity nearbyEntity = node.entity;
    ...
    private int[] getCellIndices(CollidableEntity entity, boolean isMin) {
    ...
    private void insertEntityInCells(CollidableEntity entity, Set<String> cellsToAdd) {
    ...
    private void removeEntityFromCells(CollidableEntity entity, Set<String> cellsToRemove) {
